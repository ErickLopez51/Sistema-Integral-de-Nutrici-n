...
$('#grupos1 tbody').on("click", "button.agregar1",function(){
 ...
 else
 {
   ...
   var rowNode = agregados
   .row.add( [ usuario, correo,"<div class='col text-center'><button id='"+idBoton+"' title='Eliminar usuario al grupo' class='eliminar btn btn-danger btn-circle'><i class='fas fa-trash'></i></button></div>"  ] )
   .draw()
   .node();
 }
